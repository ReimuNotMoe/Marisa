...
void SessionTCP::inline_async_write_impl() {
#ifdef DEBUG
...
std::future<std::pair<boost::system::error_code, std::shared_ptr<std::vector<uint8_t>>>> SessionTCP::read_async_promised_impl(
...
std::vector<uint8_t> SessionTCP::read_async_coro_impl(boost::asio::yield_context &__yield_ctx, size_t __buf_size) {
	...
	auto rc = tcp_socket.async_read_some(boost::asio::buffer(ret.data(), ret.size()), __yield_ctx);
	ret.resize(rc);
#ifdef DEBUG
...
void SessionTCP::read_async_with_callback_impl(
...
size_t SessionTCP::write_async_coro_impl(Buffer __data, boost::asio::yield_context &__yield_ctx) {
#ifdef DEBUG
	LogD("%s[0x%016" PRIxPTR "]:\twrite_async_coro_impl: begin\n", ModuleName, (uintptr_t)this);
#endif
	setup_timeout_timer_coro();

	auto s = boost::asio::async_write(tcp_socket, __data.get(), __yield_ctx);
#ifdef DEBUG
...
void SessionTCP::write_async_promised_impl() {
#ifdef DEBUG
...
void SessionTCP::write_async_with_callback_impl(Buffer __data,
	...
	boost::asio::async_write(tcp_socket, buf->get(), io_strand.wrap(
